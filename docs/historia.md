...
| **B6** | Logo. Alan subió `public/{logo.png, 3.png}` (ya transparentes — palette + tRNS). Generados: `src/app/{favicon.ico,icon.png,apple-icon.png}` (la marca sobre blanco; Next los linkea con basePath) + `public/logo-mark.png` (watermark del canvas al 5%, hijo de `<ReactFlow>` por el fondo opaco de `colorMode="dark"`). `src/model/assets.ts` `rutaAsset()`. `logo.png` (lockup) queda para un landing futuro. | B6 |
